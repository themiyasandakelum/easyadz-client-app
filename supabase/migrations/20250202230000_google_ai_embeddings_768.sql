-- Switch from OpenAI (1536) to Google AI (768) embeddings
-- Drop old column and add new one with correct dimensions
ALTER TABLE profiles DROP COLUMN IF EXISTS embedding;
ALTER TABLE profiles ADD COLUMN embedding vector(768);

COMMENT ON COLUMN profiles.embedding IS 'Vector embedding of bio + preferences for AI matching. Generated via Google Gemini.';

-- Update match function for 768-dimensional vectors
...
COMMENT ON FUNCTION match_matrimonial_profiles IS 'AI semantic matching using pgvector. Pass query embedding from user bio+preferences (Google Gemini).';
