-- Add verification_status to search_profiles: 'verified' | 'pending' | 'unverified'
...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN COALESCE(p.is_verified, false) THEN 'verified'::text
      WHEN v.status IN ('pending_ai', 'pending_admin') THEN 'pending'::text
